...
MongoDB.prototype.fromDatabase = function (modelName, data) {
  ...
  const modelInfo =
    ...
    this.dataSource.modelBuilder.definitions[modelName];
...
MongoDB.prototype.create = function (modelName, data, options, callback) {
  ...
  this.execute(
    ...
    'insertOne',
    data,
    addOption({ safe: true }, options, 'checkKeys'),
    function (err, result) {
      if (self.debug) {
...
MongoDB.prototype.save = function (modelName, data, options, callback) {
  ...
  this.execute(
    ...
    { _id: oid },
    { $set: data },
    addOption({ upsert: true }, options, 'checkKeys'),
    function (err, result) {
      if (!err) {
...
MongoDB.prototype.buildWhere = function (modelName, where, options) {
  ...
  Object.keys(where).forEach(function (k) {
    ...
    if (
      propDef &&
      propDef.mongodb &&
      typeof propDef.mongodb.dataType === 'string'
    ) {
...
MongoDB.prototype.buildSort = function (modelName, order, options) {
  ...
  const modelClass = this._models[modelName];
  let disableDefaultSort =
    this.settings.disableDefaultSort ||
    modelClass.settings.disableDefaultSort ||
    (options && options.disableDefaultSort);
  ...
  if (order) {
    ...
    let keys = typeof order === 'string' ? order.split(',') : order;
    ...
    sort = { _id: 1 }; // order by _id by default
  }
  ...
  params.forEach(function (near) {
    ...
    if (near.mongoKey) {
      if (Array.isArray(near.mongoKey)) {
        property = query.where || Object.create(null);
        ...
        for (i = 0; i < near.mongoKey.length; i++) {
          ...
          if (near.mongoKey.hasOwnProperty(i + 1)) {
            if (!Object.prototype.hasOwnProperty.call(property, subKey)) {
        ...
        query.where = query.where || Object.create(null);
        query.where[near.mongoKey] = queryValue;
      }
    }
  });
}
...
function errorIdNotFoundForReplace(idValue) {
  const msg =
    'Could not replace. Object with id ' + idValue + ' does not exist!';
    ...
    async.eachSeries(
      ...
      function (modelName, modelCallback) {
        ...
        self.db.dropCollection(collectionName, function (err, collection) {
          ...
          if (self.debug) {
            debug(
              'create collection %s for model %s',
              collectionName,
              modelName
            );
          }
...
function isStoredAsObjectID(propDef) {
  ...
  if (propDef.mongodb) {
    ...
    if (typeof propDef.type === 'string' && typeIsObjectId(propDef.type))
      return true;
...
function isStrictObjectIDCoercionEnabled(modelCtor, options) {
  ...
  return (
    ...
    (modelCtor.model &&
      modelCtor.model.getConnector().settings.strictObjectIDCoercion) ||
...
function visitAllProperties(data, modelCtor, visitor) {
  ...
  const modelProps = modelCtor.properties
    ? modelCtor.properties
...
function coercePropertyValue(modelCtor, propValue, propDef, setValue) {
  ...
  if (propDef && propDef.mongodb && propDef.mongodb.dataType) {
    ...
    if (Array.isArray(propValue)) {
      propValue = propValue.map((val) =>
        coerceToObjectId(modelCtor, propDef, val)
      );
    } else {
 ...
 * @param {*} connectorOptions The connector options
 * @param {*} property Connector's option's property name
 */
function addOption(commandOptions, connectorOptions, property) {
  if (!commandOptions) {
    commandOptions = {};
  }
  if (connectorOptions && connectorOptions.hasOwnProperty(property)) {
    commandOptions[property] = connectorOptions[property];
  }
